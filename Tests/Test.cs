...
namespace GmailTA.Tests
{
    ...
    public class Tests : BaseTest
    {
        ...
        public void GmailSmokeTest()
        {
            ...
            var _composeMessagePage = _mainPage.ClickComposeButton().FillFullMail(patternMessage);
            ...
            Assert.IsTrue(_draftPage.VerfiyMailVisibleInFolder(patternMessage));
            ...
            Assert.IsTrue(_composeMessagePage.IsMessageHasExpectedValuesInFields(patternMessage));
            ...
            Assert.IsFalse(_draftPage.VerfiyMailVisibleInFolder(patternMessage));
            // Step 9.Verify, that the mail is in ‘Sent’ folder.
            var _sentPage = _mainPage.ClickOnFolder<SentFolderPage>(MainPage.sentName);
            Assert.IsTrue(_sentPage.VerfiyMailVisibleInFolder(patternMessage));
            // Step 10.	Log off.
            var _chooseAnAccountPage = WebDriverExtension.NavigateToUrl<ChooseAnAccountPage>(ChooseAnAccountPage.chooseAnAccountLink);
        ...
        public void ScheduledEmails()
        {
            ...
            var _scheduledSendTab = _composeMessagePage.FillFullMail(patternMessage).ClickScheduledSendOption();
            // Step 3.	Schedule send for the mail using ‘Select Date and time’ option in ‘Schedule send’ dialog.
            _scheduledSendTab.ChooseEmailSendSchedule<ScheduledSendDialogSteps>("Pick date & time").ChooseDateAndTime(dateTime).ClickScheduledSend();
            ...
            Assert.IsTrue(_scheduledSendPage.VerfiyMailVisibleInFolder(patternMessage));
            var _scheduledMailPage = _scheduledSendPage.OpenEmailBySubjectInFolder<ScheduledMailPageSteps>(patternMessage);
            //  Step 5.	Verify the mail content(addressee, subject and body – should be the same as in step 2).
            Assert.IsTrue(_scheduledMailPage.IsOpenedMailInScheduledFolderSameAsExpected(patternMessage));
            ...
            Assert.IsFalse(_scheduledSendPage.VerfiyMailVisibleInFolder(patternMessage));
        ...
        public void StarredEmails()
        {
            ...
            _mainPage.ClickComposeButton().FillFullMail(starredMessage).LabelEmail("Add star").ClickSaveAndCloseMail();
            // Step 3. Create another draft without any label.
            Log.Debug("FillFullMail", TestsData.to + ", Message with star, Where is star?");
            _mainPage.ClickComposeButton().FillFullMail(nonStarredMessage).ClickSaveAndCloseMail();
            //  Step 4.	Verify that both mails are present in ‘Drafts’ folder.
            var _draftPage = _mainPage.ClickOnFolder<DraftsFolderPage>(MainPage.draftsName);
            Log.Debug("VerifyMailExistsInDraftFolder", "without star");
            Assert.IsTrue(_draftPage.VerfiyMailVisibleInFolder(nonStarredMessage));
            Log.Debug("VerifyMailExistsInDraftFolder", "with star");
            Assert.IsTrue(_draftPage.VerfiyMailVisibleInFolder(starredMessage));
            ...
            Assert.That(_draftPage.IsMessageSelectedBySubject(starredMessage));
            ...
            Assert.IsFalse(_draftPage.VerfiyMailVisibleInFolder(nonStarredMessage)); 
            Log.Debug("VerfiyMailAsDraft", "with star");
            Assert.IsFalse(_draftPage.VerfiyMailVisibleInFolder(starredMessage)); 
            //  Step 13. Log off.
            Log.Info("Logout");
            var _chooseAnAccountPage = WebDriverExtension.NavigateToUrl<ChooseAnAccountPage>(ChooseAnAccountPage.chooseAnAccountLink);
            _chooseAnAccountPage.ClickLogoutButton();
